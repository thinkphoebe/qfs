 ...
 * of bytes.
 */
void md5_update(md5context_t *ctx, unsigned char const *buf, size_t len)
{
    uint32_t t;
 ...
 */
void md5_final(unsigned char digest[16], md5context_t *ctx)
{
    unsigned int count;
    unsigned char *p;
...
{
    static const char num2char[] = "0123456789abcdef";
    unsigned char digest[16];
    int i;
    
    md5_digest(buf, len, digest);
    for (i = 0; i < 16; i++)
    {
        result_str[i * 2] = num2char[digest[i] >> 4];
        result_str[i * 2 + 1] = num2char[digest[i] & 0x0f];
    }
    result_str[32] = '\0';
}
